#pragma once
#include "../Libs.h"
#include "../ecs/Bush.h"
#include "../ecs/Tile.h"
#include "../utils/noise/PerlinNoise.hpp"

namespace world_generator { 
    template<glm::u32 x, glm::u32 y>
    std::array<entt::entity, x * y> make_bushes(glm::vec2 top_left, float padding, entt::registry& registry){
        auto texture = ecs::Bush::defaultTextureBundle();
        std::array<entt::entity, x * y> bushes;
        for (glm::u32 j = 0; j < y; j++){
            for (glm::u32 i = 0; i < x; i++){
                bushes[j * y + i] = ecs::Bush::newBush(glm::vec2{i * padding + top_left.x, j * padding + top_left.y}, texture, registry);
            }
        }
        return bushes;
    }

    template<config::MapConfig map_config>
    using TileMap = std::array<ecs::CTile::TileType, map_config.num_tiles_x * map_config.num_tiles_y>;

    glm::ivec2 wrapped_index(glm::ivec2 pos, glm::uvec2 dimmensions){
        if (pos.x < 0)
            pos.x = dimmensions.x - pos.x;
        else if (pos.x > dimmensions.x)
            pos.x -= dimmensions.x;
        if (pos.y < 0)
            pos.y = dimmensions.y - pos.y;
        else if (pos.y > dimmensions.y)
            pos.y -= dimmensions.y;
        return pos;
    }

    template<config::MapConfig map_config>
    inline ecs::CTile::TileType get_val(TileMap<map_config>& tile_map, glm::ivec2 pos){
        return tile_map[wrapped_index(pos, {map_config.num_tiles_x, map_config.num_tiles_y})];
    }

    template<config::MapConfig map_config>
    glm::u8 check_around(TileMap<map_config>& tiles,int x, int y){
        glm::u8 result = 0;
        result |= get_val<map_config>(tiles, {x-1, y+1});
    }
   
    template<config::MapConfig map_config>
    static void generateGround(entt::registry& registry){
        //TileMap<map_config> tiles;
        glm::vec2 origin = {map_config.num_tiles_x * config::render_tile_size / -2.0f, map_config.num_tiles_y * config::render_tile_size / -2.0f};
        auto coordinate = [origin](int x, int y) {
            return glm::vec2(origin.x + x * config::render_tile_size, origin.y + y * config::render_tile_size);
        };
        auto texture = ecs::Tile::defaultTextureBundle();
        for (size_t y = 0; y < map_config.num_tiles_y; y++){
            for (size_t x = 0; x < map_config.num_tiles_x; x++){
                float noise_value = utils::noise::octave_perlin_noise(x, y, 1,1,12);     
                if (noise_value < 0.f){
                    texture.src.key = ecs::TextureKeys::Water;
                    texture.src.rect = ecs::textures::water_0;
                    ecs::Tile::newTile(coordinate(x,y), texture, registry, ecs::CTile::TileType::Water);
          //          tiles[y * map_config.num_tiles_x + x] =  ecs::CTile::TileType::Water;
                    if (noise_value > 1.0f){
                        ecs::Bush::newBush(coordinate(x,y), ecs::Bush::defaultTextureBundle(), registry);
                    }
                }else{
                    texture.src.key = ecs::TextureKeys::Grass;
                    texture.src.rect = ecs::textures::grass_0;
                    ecs::Tile::newTile(coordinate(x,y), texture, registry, ecs::CTile::TileType::Grass);
          //          tiles[y * map_config.num_tiles_x + x] =  ecs::CTile::TileType::Grass;
                }
            }
        }
        /*for (size_t y = 0; y < map_config.num_tiles_y; y++){
            for (size_t x = 0; x < map_config.num_tiles_x; x++){
                int bits = check_around<map_config>(tiles, x, y);
            }
        }*/

    }
}
