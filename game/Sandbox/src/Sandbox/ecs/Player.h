...
namespace Sandbox::ecs::Player {
    ...
    inline std::tuple<QuadTexInstancedData> on_create(Syris::EntityManager &entity_manager, entt::entity player,
                                                      glm::vec2&& pos, MovementKeys keys)
    {

        static QuadTexInstancedData instance_data{
            {texture::atlas::player_0.min, texture::atlas::player_0.max},
            glm::mat4(1.f)};
        ...
        entt::registry &registry = entity_manager.get_registry();
        registry.emplace<AsyncComponent<CPosition>>(player, pos);
        registry.emplace<CTexture>(player, instance_data.tex_coord);
        registry.emplace<CKeyBinded>(player, keys);
        registry.emplace<CSpeed>(player, glm::vec2(0.f));
        registry.emplace<CMovementSpeed>(player, 3.f);
        registry.emplace<CDir>(player, glm::vec2(0.f, -1.f));
