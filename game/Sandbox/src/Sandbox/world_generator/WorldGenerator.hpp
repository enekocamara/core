...
//        noise.SetCellularDistanceFunction(FastNoiseLite::CellularDistanceFunction::CellularDistanceFunction_Euclidean);
        ...
        auto tile_gen = [noise](glm::uvec2 pos, Syris::EntityManager& entity_manager, uint32_t index) -> entt::entity
        {
            ...
            if (noise_value < 0)
            {
                return ecs::Tile::newTile(glm::vec2(pos), texture::atlas::dirt_0, entity_manager, ecs::CTile::TileType::Water, index,0);
            }
            else
            {
                return ecs::Tile::newTile(glm::vec2(pos), texture::atlas::grass_0,entity_manager , ecs::CTile::TileType::Grass, index, noise_value * 40);
            }
        };
